function [new_lb,branch_name,matrix,hit]=findlb_ball(p,matrix_name,matrix_store)
...
while wucha>0.1
    ...
    f=0.5*lambda*sum(element_list2(index))-0.5*(u*e-n*p_mean')'*m'+n*expect_return-u;
    wucha=abs(f-best_lb);
    if best_lb<f
        best_lb=f;
    end
    is_name2=ismember(pf1_index,pf_index(index));
    min_diag=eye(length(p_mean));
    min_diag=diag(is_name2*min_diag);
    d1=lambda*m*min_diag*ai-m*e-1;
    d2=-lambda*m*min_diag*pi+m*p_mean'+expect_return;
    u=u+s1*d1;
    n=n+s2*d2;
    if n<0
        n=0;
